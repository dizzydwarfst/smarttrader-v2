...
export default function ControlPanel({ controlStatus, onRefresh }) {
  ...
  return (
    <div data-testid="control-panel" className="rounded-xl p-6" style={{ background: '#FFFFFF', border: '1px solid #E2E8F0' }}>
      <div className="flex items-center justify-between mb-5 cursor-pointer" onClick={() => setExpanded(!expanded)}>
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-[16px] font-bold" style={{ color: '#0F172A', fontFamily: 'Outfit, sans-serif' }}>Control Panel</h3>
        ...
        {expanded ? <ChevronUp className="w-5 h-5" style={{ color: '#94A3B8' }} /> : <ChevronDown className="w-5 h-5" style={{ color: '#94A3B8' }} />}
      </div>
        ...
        {['conservative', 'routine', 'aggressive', 'scalper'].map(p => {
          ...
          return (
            <button key={p} data-testid={`profile-btn-${p}`} onClick={() => activateProfile(p)}
              ...
              style={{
                background: act ? '#2563EB' : '#F1F5F9',
                color: act ? '#F8FAFC' : '#475569',
                border: `1px solid ${act ? '#2563EB' : '#E2E8F0'}`
              }}
              onMouseEnter={(e) => { if (!act) { e.currentTarget.style.borderColor = '#2563EB'; e.currentTarget.style.color = '#0F172A'; } }}
              onMouseLeave={(e) => { if (!act) { e.currentTarget.style.borderColor = '#E2E8F0'; e.currentTarget.style.color = '#475569'; } }}>
              {p.charAt(0).toUpperCase() + p.slice(1)}
            </button>
          );
        })}
      </div>
      <p className="text-[14px]" style={{ color: '#475569' }}>{isCustom ? 'Custom settings applied.' : profileDescriptions[currentProfile]}</p>

      {expanded && (
        <div className="space-y-4 pt-5 mt-5 border-t" style={{ borderColor: '#E2E8F0' }}>
          <div className="flex flex-wrap items-center gap-3 p-4 rounded-lg" style={{ background: '#F8FAFC', border: '1px solid #E2E8F0' }}>
            ...
            <button data-testid="apply-custom-btn" onClick={applyCustom}
              ...
              onMouseLeave={(e) => e.currentTarget.style.background = '#2563EB'}>Apply</button>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <button data-testid="set-min-pnl-btn" onClick={applyMinPnl}
                ...
                onMouseLeave={(e) => { e.currentTarget.style.borderColor = '#E2E8F0'; e.currentTarget.style.color = '#475569'; }}>Set</button>
            </div>
          </div>
          {reverseMode && (
            <div data-testid="reverse-mode-notice" className="p-4 rounded-lg text-[13px]"
              style={{ background: 'rgba(220,38,38,0.08)', color: '#FCA5A5', border: '1px solid rgba(220,38,38,0.25)' }}>
              <strong style={{ color: '#DC2626' }}>Reverse Mode Active</strong> — BUY signals execute as SELL, and vice versa.
            </div>
          )}
        </div>
      )}
